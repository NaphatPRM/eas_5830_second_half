...
contract AMM is AccessControl{
	...
	*/
    ...
    // Assign the value on qtyA and qtyB
    qtyA = ERC20(tokenA).balanceOf(address(this));
    qtyB = ERC20(tokenB).balanceOf(address(this));
    // Assign the delta based on the feebps
    swapAmt = sellAmount * (10000 - feebps) / 10000;
		// YOUR CODE HERE 
    if (sellToken == tokenA) {
      uint256 newA = qtyA + swapAmt;
      uint256 newB = invariant / newA;
      uint256 retB = qtyB - newB;
      require(retB > 0, "return value must be more than 0 for B");

      ERC20(tokenA).transferFrom(msg.sender, address(this), sellAmount);
      ERC20(tokenB).transfer(msg.sender, retB);

      emit Swap(tokenA, tokenB, swapAmt, retB);
    } 

    if (sellToken == tokenB) {
      uint256 newB = qtyB + swapAmt;
      uint256 newA = invariant / newB;
      uint256 retA = qtyA - newA;
      require(retA > 0, "return value must be more than 0 for A");

      ERC20(tokenA).transfer(msg.sender, retA);
      ERC20(tokenB).transferFrom(msg.sender, address(this), sellAmount);

      emit Swap(tokenB, tokenA, swapAmt, retA);
    } 
	...
	*/
    ...
    // Copy what withdrawLiquidity does
